...
_val = _group getVariable "aiMaster_fightVeh";
if ( isNil "_val" ) then {
	_group setVariable ["aiMaster_fightVeh", [false, nil, nil]];
};
...
if ( !isNil "_time" ) exitWith 
{
	...
	if ( _passed > 60 ) exitWith {
		...
		if !( isNull _enemySpotted ) then {
			if ( _enemySpotted distance _leader < 500 ) exitWith { };
		};
		_group setVariable ["aiMaster_fightVeh", [_offensive, _time, _oldLoc]];
		if ( _group in aiMaster_fightingGroupsVeh ) then {
			aiMaster_fightingGroupsVeh = aiMaster_fightingGroupsVeh - [ _group ];
...
if ((visiblePosition _enemySpotted select 2) > 10) exitWith { };
...
if (_loc distance [0,0,0] < 100 || _loc distance _leader > 1500) exitWith { };
_dist = 50 + (random 300);
_wpPos = [ _loc, _dist ] call _findPos;
if (_wpPos distance [0,0,0] < 100 || _wpPos distance _leader > 1500) exitWith { };
...
_group setVariable ["aiMaster_fightVeh", [_offensive, time, _loc]];
true
