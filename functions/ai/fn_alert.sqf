...
_var = _group getVariable "aiMaster_patrol";
_loc = _var select 0;
_dist = _var select 1;
...
_val = _group getVariable "aiMaster_alert";
if ( isNil "_val" ) then {
	_group setVariable ["aiMaster_alert",[false,0]];
	_val = [false,0];
};
...
if ( _alert ) exitWith {
	...
	if ( _passed > 600 ) then {
		...
		_group setVariable ["aiMaster_alert",[false, 0]];
		_x setVariable ["aiMaster_patrol",[_loc,_dist, [ _building, _oldStance, _speed ],_out]];
	};
};
...
_group setVariable ["aiMaster_alert",[true,time]];
